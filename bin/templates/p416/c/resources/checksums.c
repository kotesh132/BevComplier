...
void verify_checksum(bool condition, uint8_t *buffer, uint8_t *checksum, int length, HashAlgorithm algo){
    ...
    if (condition) {
        switch (algo) {
            ...
            {
                uint16_t result = 0;
                crc16_fn(buffer, length, (uint8_t *)&result);
                if (result != *(uint16_t *)checksum){
                    printf("\nverify checksum16 invoked\n");
                    //mark_to_drop();
                }
                break;
            }
            ...
            {
                uint32_t result = 0;
                crc32_fn(buffer, length, (uint8_t *)&result);
                if (result != *(uint32_t *)checksum){
                    printf("\nverify checksum32 invoked\n");
                    //mark_to_drop();
                }
                break;
            }
...
void update_checksum(bool condition, uint8_t *buffer, uint8_t *checksum, int length, HashAlgorithm algo){
