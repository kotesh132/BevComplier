...
public final class PrettyTablePrinter {
    ...
    public PrettyTablePrinter(PrintStream out, String asNull) {
        if (out == null) {
    ...
    public String emit(String[][] table) {
        if (table == null) {
            throw new IllegalArgumentException("No tabular data provided");
        }
        if (table.length == 0) {
        ...
        final int[] widths = new int[getMaxColumns(table)];
        adjustColumnWidths(table, widths);
        return emitPreparedTable(table, widths, getHorizontalBorder(widths));
    }

    private String emitPreparedTable(String[][] table, int widths[], String horizontalBorder) {
        final int lineLength = horizontalBorder.length();
        StringBuilder sb = new StringBuilder();
        sb.append(horizontalBorder+"\n");
        for (final String[] row : table) {
            if (row != null) {
                sb.append(getRow(row, widths, lineLength)+"\n");
                sb.append(horizontalBorder+"\n");
            }
        }
        return sb.toString();
