...
@Data
public class Extractor implements Configurable {
    ...
    public List<Bitset> summary() {
        Bitset vbitList = ApiUtils.concat(ForgeSpec.EXCTRACTOR_VBIT_BIT_SIZE, Utils.listTokenizerToBitsetList(vbit));
        Bitset cbitList = ApiUtils.concat(ForgeSpec.EXCTRACTOR_CBIT_BIT_SIZE, Utils.listTokenizerToBitsetList(cbit));
        Bitset xbitList = ApiUtils.concat(ForgeSpec.EXCTRACTOR_XBIT_BIT_SIZE, Utils.listTokenizerToBitsetList(xbit));
        Bitset ybitList = ApiUtils.concat(ForgeSpec.EXCTRACTOR_YBIT_BIT_SIZE, Utils.listTokenizerToBitsetList(ybit));
        Bitset vbytList = ApiUtils.concat(ForgeSpec.EXCTRACTOR_VBYT_BIT_SIZE, Utils.listTokenizerToBitsetList(vbyt));
        Bitset cbytList = ApiUtils.concat(ForgeSpec.EXCTRACTOR_CBYT_BIT_SIZE, Utils.listTokenizerToBitsetList(cbyt));
        Bitset xbytList = ApiUtils.concat(ForgeSpec.EXCTRACTOR_XBYT_BIT_SIZE, Utils.listTokenizerToBitsetList(xbyt));
        Bitset ybytList = ApiUtils.concat(ForgeSpec.EXCTRACTOR_YBYT_BIT_SIZE, Utils.listTokenizerToBitsetList(ybyt));
        Bitset ymskList = ApiUtils.concat(ForgeSpec.EXCTRACTOR_YMSK_BIT_SIZE, Utils.listTokenizerToBitsetList(ymsk));
        Bitset xsftList = ApiUtils.concat(ForgeSpec.EXCTRACTOR_XSFT_BIT_SIZE, Utils.listTokenizerToBitsetList(xsft));
        Bitset xdirList = ApiUtils.concat(ForgeSpec.EXCTRACTOR_XDIR_BIT_SIZE, Utils.listTokenizerToBitsetList(xdir));
        return ConfigApis.concat_fields_extractor_extractor_ctrl_cfg_0(vbitList, cbitList, xbitList, ybitList,
    ...
    public void generate() {
        ...
        Extractor exRow = Extractor.getInstance();
        List<PIEntry> exRows = new ArrayList<PIEntry>();
        ...
        for (ExtractorConfigUnit configs : allConfigs) {
            ...
            parser.setEx(Utils.flattenNestedList(records));
            config.setPa(parser);
