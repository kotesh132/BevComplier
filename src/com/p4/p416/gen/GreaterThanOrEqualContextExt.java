...
import com.p4.p416.gen.P416Parser.GreaterThanOrEqualContext;

public class GreaterThanOrEqualContextExt extends ExpressionContextExt {
	
	private static final Logger L = LoggerFactory.getLogger(GreaterThanOrEqualContextExt.class);
	
	private String thisTypeName;

	private int thisTypeSize;

	public GreaterThanOrEqualContextExt(GreaterThanOrEqualContext ctx) {
		super(ctx);
	}

	@Override
	public  GreaterThanOrEqualContext getContext(){
		return (GreaterThanOrEqualContext)contexts.get(contexts.size()-1);
	}

	@Override
	public GreaterThanOrEqualContext getContext(String str){
		return (GreaterThanOrEqualContext)new PopulateExtendedContextVisitor().visit(getParser(str).expression());
	}
	...
	public void setContext(ParserRuleContext ctx){
		if(ctx != null){
			if(ctx instanceof GreaterThanOrEqualContext){
				...
				throw new ClassCastException(ctx.getClass().getSimpleName() + " cannot be casted to "+ GreaterThanOrEqualContext.class.getName());
			}
		} else {
			addToContexts(null);
		}
	...
	public String getOperator() {
		return getContext().GE().getText().trim();
	}
	
	@Override
	public Type getType(){
		ExpressionContextExt leftExpression = (ExpressionContextExt) getExtendedContext(((GreaterThanOrEqualContext)getContext()).expression(0));
		ExpressionContextExt rightExpression = (ExpressionContextExt) getExtendedContext(((GreaterThanOrEqualContext)getContext()).expression(1));
