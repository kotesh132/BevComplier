...
public class PrefixedNonTypeNameLvalueContextExt extends LvalueContextExt {

	public PrefixedNonTypeNameLvalueContextExt(PrefixedNonTypeNameLvalueContext ctx) {
		super(ctx);
	}

	@Override
	public  PrefixedNonTypeNameLvalueContext getContext(){
		return (PrefixedNonTypeNameLvalueContext)contexts.get(contexts.size()-1);
	}

	@Override
	public PrefixedNonTypeNameLvalueContext getContext(String str){
		return (PrefixedNonTypeNameLvalueContext)new PopulateExtendedContextVisitor().visit(getParser(str).lvalue());
	}

	@Override
	public void setContext(ParserRuleContext ctx){
		if(ctx != null){
			if(ctx instanceof PrefixedNonTypeNameLvalueContext){
				addToContexts(ctx);
			} else {
				throw new ClassCastException(ctx.getClass().getSimpleName() + " cannot be casted to "+ PrefixedNonTypeNameLvalueContext.class.getName());
			}
		} else {
			addToContexts(null);
		}
	...
	public void markDelete(SsaForm obj){
		try{
			...
			if(defSymbol.doneDelete==true) return;
			defSymbol.doneDelete=true;
	...
	public void getSSAFormattedText(Params params){
		...
		if(this.ssaVersion>0)
			params.getStringBuilder().append(getExtendedContext(getContext()).getFormattedText()+"("+this.ssaVersion+")");
		else
			params.getStringBuilder().append(getExtendedContext(getContext()).getFormattedText());
