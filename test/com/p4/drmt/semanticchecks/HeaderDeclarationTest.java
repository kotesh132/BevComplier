...
public class HeaderDeclarationTest {

	private static  Logger L = LoggerFactory.getLogger(ExternDeclarationTest.class);
	final static SemanticTestDataSet semanticTestDataSet = SemanticUtils.readJson("test/com/p4/drmt/semanticchecks/data/HeaderDeclaration.json");

	private SemanticTestData semTestData;

	public HeaderDeclarationTest(SemanticTestData data) {
		semTestData = data;
	}

	@Test
	public void semanticCheckForHeaderDeclarationValidation(){
		L.info("Running Junit semanticCheckForHeaderDeclaration");
		Assert.assertTrue(SemanticUtils.compareData(semTestData,new HeaderDeclarationVisitor()));
